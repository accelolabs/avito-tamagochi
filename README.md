...
3. Зарядка добавляет 20% энергии, а первая зарядка дня увеличивает серию и дневную XP-награду.
4. Зарядка, ежедневное поглаживание и задания начисляют XP, а новые уровни открывают награды.
...
Mailpit перехватывает письма локально и не доставляет их в Gmail или другие реальные почтовые ящики. Пакетная команда проверяет всех зарегистрированных пользователей с существующим питомцем:

```bash
docker compose --profile tools run --rm --no-deps notifications
```

По умолчанию UI Mailpit доступен на <http://localhost:8025>. Порт и адрес отправителя настраиваются через `MAILPIT_UI_PORT` и `MAIL_FROM` в `.env`. Отправленные пороги сохраняются в PostgreSQL и не повторяются, пока зарядка не поднимет энергию выше соответствующего порога.
